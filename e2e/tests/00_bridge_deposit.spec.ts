...
for (const destinationChainId of Object.keys(originRoutesToTest)) {
  testWithConnectedMM(
    ...
    async ({ page, metamask }) => {
      ...
      const chain = chains[depositChainIdToTest];
      await metamask.addNetwork({
        name: chain.name,
        rpcUrl: chain.rpcUrl,
        chainId: depositChainIdToTest,
        symbol: "ETH",
      });
...
for (const originChainId of Object.keys(destinationRoutesToTest)) {
  ...
  testWithConnectedMM(
    ...
    async ({ page, metamask }) => {
      ...
      const chain = chains[originChainId];
      await metamask.addNetwork({
        name: chain.name,
        rpcUrl: chain.rpcUrl,
      ...
      await testDepositFlow(
        ...
        destinationRoutesToTest[originChainId],
        tokenAmountsMap,
        page,
        metamask as any
      );
...
async function testDepositFlow(
  ...
  const toChainIds = new Set(routes.map(({ toChain }) => toChain));
