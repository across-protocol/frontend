...
export function useBlock(chainId?: ChainId) {
	...
	const queryKey = enabledQuery ? latestBlockQueryKey(chainId) : "DISABLED_BLOCK_QUERY";
	const refetchInterval = enabledQuery ? (CHAINS[chainId].pollingInterval ?? DEFAULT_BLOCK_POLLING_INTERVAL) : undefined;
	const { data: block, ...delegated } = useQuery(queryKey, async () => {
 ...
 */
async function getBlock(chainId: ChainId, blockHashOrBlockTag: ethers.providers.BlockTag = "latest"): Promise<ethers.providers.Block> {
