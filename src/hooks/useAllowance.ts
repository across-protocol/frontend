 ...
 */
export function useAllowance(token: string, chainId?: ChainId, owner?: string, spender?: string, blockNumber?: number) {
	...
	const enabledQuery = !!chainIdToQuery && !!ownerToQuery && !!spender && !!blockNumberToQuery;
	const queryKey = enabledQuery ? allowanceQueryKey(chainIdToQuery, token, ownerToQuery, spender, blockNumberToQuery) : ["DISABLED_ALLOWANCE_QUERY", { chainIdToQuery, token, ownerToQuery, spender, blockNumberToQuery }];
	const { data: allowance, ...delegated } = useQuery(queryKey, async () => {
		const allowance = await getAllowance(chainId!, token, ownerToQuery!, spender!, blockNumberToQuery);
		return allowance;
	}, {
